...
def load_data(split_ratio):
    table = xlrd.open_workbook(filename).sheets()[0]  # 获取第一个sheet表
    row = table.nrows  # 行数
    col = table.ncols  # 列数
    ...
    tmp = np.hstack((datamatrix_feature_age, datamatrix_feature_gender,datamatrix_label))

    # tmp=np.load("~/Desktop/day_in_the_life_after_processing_numberLabe.xls")
    data=tmp[:,:-1]
    label=tmp[:,-1]
    # print(data,label)
